...
# http://github.com/jupyter/docker-stacks
	@echo "illumidesk/jupyter-tabula-proxy"
	@echo "====================="
...
build: ## build the latest image for a stack
	${VENV_BIN}/jupyter-repo2docker --no-run --user-id 1000 --user-name jovyan --image-name $(OWNER)/tabula:$(TAG) .
	@echo -n "Built image size: "
	@docker images $(OWNER)/tabula:$(TAG) --format "{{.Size}}"
